<?php
 ...
 if(isset($_POST['delete']))
 {
    ...
    $r = mysqli_query($conn, "SELECT * FROM admin_login WHERE name = '$name'") or die(mysqli_error($conn));
    ...
    if (empty($multi_id)) {
        echo "<script>alert('Please select files to delete!'); window.location='add_document.php';</script>";
        exit;
    }
            ...
            if (file_exists($dir . '/' . $file_name)) {
                ...
                if (rename($dir . '/' . $file_name, $archiveFolder . '/' . $new_file_name)) {
                    ...
                    if (mysqli_query($conn, $sql)) {
                        echo '<script>alert("File archived successfully!"); window.location="add_document.php";</script>';
                    } else {
                        echo '<script>alert("Error: ' . mysqli_error($conn) . '"); window.location="add_document.php";</script>'; // Handle SQL insert error
                    }
                } else {
                    echo '<script>alert("Failed to move file to archive folder!"); window.location="add_document.php";</script>';
                }
            } else {
                echo '<script>alert("File ' . $file_name . ' not found in directory!"); window.location="add_document.php";</script>';
            }
        } else {
            echo '<script>alert("File record not found or Files not yours!"); window.location="add_document.php";</script>';
        }
    }
