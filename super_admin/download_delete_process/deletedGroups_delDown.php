 ...
 if (isset($_POST['delete'])) {
    ...
    if (empty($_POST['checkbox'])) {
        ...
        foreach ($_POST['checkbox'] as $folder_id) {
            ...
            $query = mysqli_query($conn, "SELECT * FROM `archive_group_folder` WHERE id='$folder_id'");
            if ($query && mysqli_num_rows($query) > 0) {
                $folder = mysqli_fetch_assoc($query);
                $folder_name = $folder['folder_name'];
                $folder_path = "../../archived/deleted_groups/" . $folder_name;

                // Check if the folder exists
                if (is_dir($folder_path)) {
                    // Remove the folder and its contents recursively
                    mysqli_query($conn, "DELETE FROM `archive_group_folder` WHERE id='$folder_id'");

                    deleteFolder($folder_path);
                    echo "<script>alert('the archived folder has been deleted'); window.location='../view_deleted_group.php';</script>";
