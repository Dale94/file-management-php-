...
<tbody>
    ...
    <tr>
      ...
      $query = mysqli_query($conn,"SELECT ID,NAME,SIZE,EMAIL,ADMIN_STATUS,TIMERS,DOWNLOAD FROM upload_files WHERE NAME != 'EMPTY' group by NAME DESC") or die (mysqli_error($conn));
  ...
  <div class="modal-dialog" role="document">
    ...
    <div class="modal-content">
      <div class="modal-header text-center">
        ...
        <!-- <div class="md-form mb-4">
          <i class="fas fa-lock prefix grey-text"></i>
          <input type="password" id="orangeForm-pass" name="confimation_password" class="form-control validate">
          <label for="orangeForm-pass">Confirmation password</label>
        </div>
      </div> -->
      <div class="modal-footer d-flex justify-content-center">
 ...
 if(isset($_POST['edit'])){
  ...
  if (password_verify($current_password, $pass1)) {
    ...
    echo "<script type = 'text/javascript'>alert('You have update your account');document.location='home.php'</script>";
     ...
     mysqli_query($conn,"UPDATE `login_user` SET `name` = '$user_name', `email_address` = '$email_address' where id='$user_id'") or die (mysqli_error($conn));
